...
/*
 *	Advanced Custom Fields - Flickr
 *
 *	Author: Paul Huisman
 *  Author website: www.paulhuisman-online.nl
 *
 */
...
class Flickr_field extends acf_Field
{
			...
			<td class="label">
				...
				<p class="description">
					<?php if (!empty($flick_api_key)):
						_e('You already have a Flickr API key.');
					else: ?>
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		$f->enableCache("fs", dirname(__FILE__) . '/phpFlickr/cache');		
					...
					switch($field['flickr_content']) {
						case 'galleries':
							...
							if (is_array($flickr_data) && !empty($flickr_data)) {
								...
								?><tr class="field_label">
									<td colspan="2"><?php _e('There are no Flickr galleries available for user ID'); ?> <?php echo $user_id; ?></td>
								</tr><?php
							}
							break;
						...
						default:
							...
							if (is_array($flickr_data) && !empty($flickr_data)) {
								...
								?><tr class="field_label">
									<td colspan="2"><?php _e('There are no Flickr sets available for user ID'); ?> <?php echo $user_id; ?></td>
								</tr><?php
							}							
							break;
					}
	...
	*-------------------------------------------------------------------------------------*/
	...
	function admin_head()
	{
		...
		<style type="text/css">
			...
			.field_label .label.set_image	  { width:75px; }
			...
			.flickr_row .set_title		  	  { margin:0 0 8px 0; font-weight:bold; padding:0; font-size:13px; color:#444; }
		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				$('.flickr_field').each(function() {
					...
					$('a#deselect_set', self).click(function() {
						alert('moo');
						$('.flickr_row', self).removeClass('active-row');
						input.val('');
					});
				...
				var list = $('.more-sets');
				list.hide();
				$('a#show-more-sets').click(function() {
